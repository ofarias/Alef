...
//	$fecha = $_POST['fecha'];       
	...
	$controller->CancelaPreorden($id,$cotizacion,$partida,$motivo);
		...
		$empaque=$_POST['noemp'];
		$idcaja=$_POST['idcaja'];
        $tipopaq = $_POST['tipoemp'];       //23062016
        $docf = $_POST['docf'];
		$controller->AsignaEmpaque($idpreoc, $empaque, $idcaja, $tipopaq, $docf);
}elseif(isset($_POST['embalaje'])){
		$docf=$_POST['docf'];
		$caja=$_POST['caja'];
		$controller->embalaje($docf, $caja);
...
}elseif (isset($_POST['aSec'])) {
	$idcaja = $_POST['idcaja'];
	$sec = $_POST['sec'];
	$tipo = $_POST['aSec'];
	$response = $controller->aSec($idcaja, $sec, $tipo);
	echo json_encode($response);
	exit();
    ...
    $unidad = $_POST['unidad'];
    $tipo = $_POST['tipo'];
    $controller->ImprimirSecuenciaEnt($unidad, $tipo);
...
elseif (isset($_POST['imprimeValidacion'])) {
	...
	$doco = $_POST['doco'];
	$partida = $_POST['partida'];
	$response=$controller->recibeParOC($cantidad, $idp, $doco, $partida);
	echo json_encode($response);
	exit();
	...
	$response=$controller->recibirParOC($idp, $cantidad, $doco, $partida, $desc1, $desc2, $desc3, $descf, $desc1M, $desc2M, $desc3M, $descfM, $precioLista, $totalCosto, $totalPartida);
	echo json_encode($response);
	exit();
	...
	$tipo = $_POST['tipo'];
	$origen = $_POST['origen'];
	$response = $controller->procesarDev($idp, $cantDev, $tipo, $origen);
	echo json_encode($response);
	exit();
	...
	echo json_encode($response);
	exit();
}elseif (isset($_POST['cierreInvEmpaque'])){
	$tipo=$_POST['tipo'];
	$fecha = $_POST['fecha'];
	$controller->cierreInvEmpaque($fecha, $tipo);
}elseif (isset($_POST['actCanti'])) {
	$idpreoc = $_POST['idpreoc'];
	$response= $controller->actCanti($idpreoc);
	echo json_encode($response);
	exit();
}elseif (isset($_POST['impFoliosAsignacion'])) {
	$folios = $_POST['folios'];
	var_dump($folios);
	$controller->impFoliosAsignacion($folios);
}elseif (isset($_POST['impFolioReciboBodega'])) {
	$doco = $_POST['doco'];
	$controller->impFolioReciboBodega($doco);
}elseif(isset($_POST['actPreoc'])){
	$idpreoc = $_POST['idpreoc'];
	$response=$controller->actPreoc($idpreoc);
	echo json_encode($response);
	exit();
	...
	$idsol = $_POST['idsol'];
	$tipo=$_POST['tipo'];
	$response=$controller->solRefac($idsol, $tipo);
	echo json_encode($response);
	exit();
}elseif(isset($_POST['quitarPartidaRef'])){
	$docref = $_POST['docref'];
	$partida = $_POST['partida'];
	$response=$controller->quitarPartidaRef($docref, $partida);
	echo json_encode($response);
	exit();
}elseif (isset($_POST['quitarCaja'])) {
	$caja = $_POST['caja'];
	$controller->quitarCaja($caja);
	echo json_encode($controller);
	exit();
}elseif(isset($_POST['grabarRemisiones'])){
	$controller->grabarRemisiones();
}elseif (isset($_POST['recibeDocRep'])) {
	$idcaja = $_POST['caja'];
	$response=$controller->recibeDocRep($idcaja);
	echo json_encode($response);
	exit();
}elseif(isset($_POST['aRutaRep'])){
	$idcaja=$_POST['caja'];
	$unidad = $_POST['unidad'];
	$response =$controller->aRutaRep($idcaja, $unidad);
	echo json_encode($response);
	exit();
}elseif (isset($_POST['traeUnidadActual'])) {
	$idcaja = $_POST['traeUnidadActual'];
	$response=$controller->traeUnidadActual($idcaja);
	echo json_encode($response);
	exit();
}elseif (isset($_POST['finRutaLog'])) {
	$idcaja=$_POST['idcaja'];
	$fin = $_POST['fin'];
	$tipo = $_POST['finRutaLog'];
	$response=$controller->finRutaLog($idcaja, $fin, $tipo);
	echo json_encode($response);
	exit();
	...
	$idc = $_POST['idc'];
	$info = $_POST['info'];
	$response=$controller->grabaCtrCbo($idc, $info);
	echo json_encode($response);
	exit();
}elseif (isset($_POST['caratula'])) {
	$cl=$_POST['cl'];
	$fecha =$_POST['fecha'];
	$idc = $_POST['idc'];
	$response =$controller->caratula2($cl, $fecha, $idc);
	echo json_encode($response);
	exit();
}elseif (isset($_POST['invFisBod'])) {
	$prod = $_POST['prod'];
	$canto = $_POST['canto'];
	$cantn = $_POST['cantn'];
	$um = $_POST['um'];
	$response = $controller->invFisBod($prod, $canto, $cantn, $um);
	echo json_encode($response);
	exit();
}elseif (isset($_POST['buscaID'])) {
	$id=$_POST['buscaID'];
	$response=$controller->buscaID($id);
	echo json_encode($response);
	exit();
}elseif(isset($_POST['ejecutaOC'])){
	$oc=$_POST['oc'];
	$tipo = $_POST['tipo'];
	$motivo = $_POST['motivo'];
	$partida = $_POST['partida'];
	$final = $_POST['final'];
	$response = $controller->ejecutaOC($oc, $tipo, $motivo, $partida, $final);
	echo json_encode($response);
	exit();
}elseif(isset($_POST['quitarOciImp'])){
	$oc=$_POST['folio'];
	$response=$controller->quitarOciImp($oc);
	echo json_encode($response);
	exit();
...
exit();
	...
	case 'SecUnidadRec':
		$unidad = $_GET['ids'];
		$tipo =$_GET['tipo'];
		$controller->AsignaSecuenciaRec($unidad, $tipo);
		break;
	case 'subfallidos':
    ...
    case 'verCR10':                           //3006
        ...
        case 'IngresoBodega':
        	...
        	if(isset($_GET['ingresar'])){
        		$ingresar = htmlentities($_GET['ingresar']);
        	}else{
        		$ingresar = 0;
        	}
		...
		case 'invPatio':
			$controller->invPatio();
			break;
		case 'invPatioDetalle':
			$caja = $_GET['caja'];
			$controller->invPatioDetalle($caja);
			break;
		case 'verValesPatio':
		...
		case 'cierreInvEmpaque':
			...
			echo $tipo.'<br/>';
			echo $fecha.' '.$nombre;
			$controller->cierreInvEmpaque($fecha, $tipo, $nombre);
			break;
		case 'verDevProv':
			$controller->verDevProv();
			break;
		case 'verMerma':
			$controller->verMerma();
			break;
		case 'verificaIDS':
			$controller->verificaIDS();
			break;
		case 'invPatioGral':
			if(isset($_GET['opcion'])){
				$opcion = $_GET['opcion'];	
			...
			$controller->invPatioGral($opcion);
			break;
		case 'verRefacNueva':
			$docref = $_GET['docref'];
			$controller->verRefacNueva($docref);
			break;
		case 'invBodegaGral':
			$controller->invBodegaGral();
			break;
		case 'verFacturaCompleta':
			$docf = $_GET['docf'];
			$controller->verFacturaCompleta($docf);
			break;
		case 'verInvRemisiones':
			if(isset($_GET['mes'])){
				$mes = $_GET['mes'];
			}else{
				$mes = 0;
			...
			$controller->verInvRemisiones($mes, $anio, $tipo);
			break;
		case 'edoResultado':
			if(isset($_GET['mes'])){
				$mes = $_GET['mes'];
			...
			$controller->edoResultado($mes, $anio, $tipo);
			break;
		case 'verCtrlCajasAbiertas':
			$controller->verCtrlCajasAbiertas();
			break;
		case 'verFaltantesFacturar':
		...
		case 'verVueltas':
			$idcaja = $_GET['idcaja'];
			$controller->verVueltas($idcaja);
			break;
		case 'verSolRefacturacion':
			$controller->verSolRefacturacion();
			break;
		case 'seguimientoCajas':
			$controller->seguimientoCajas();
			break;
		case 'detalleCajasMensual':
			$mes = $_GET['mes'];
			$anio = $_GET['anio'];
			$controller->detalleCajasMensual($mes, $anio);
			break;
		case 'seguimientoCajasDiaDetalle':
			$mes = $_GET['mes'];
			$anio = $_GET['anio'];
			$dia = $_GET['dia'];
			$controller->seguimientoCajasDiaDetalle($mes, $anio, $dia);
			break;
		case 'verComprobantesRecibo':
			$idc = $_GET['idc'];
			$controller->verComprobantesRecibo($idc);
			break;
		case 'verOC':
			$tipo = $_GET['tipo'];
			$fecha = $_GET['fecha'];
			$controller->verOC($tipo, $fecha);
			break;
		case 'verOCmes':
			$controller->verOCmes();
			break;
		case 'verDocSinProcesar':
			$controller->verDocSinProcesar();
			break;
		case 'verHC':
			$idc = $_GET['idc'];
		...
		case 'caratula':
			...
			$fecha=$_GET['fecha'];
			$idc=$_GET['idc'];
			$controller->caratula($cl, $fecha, $idc);
			break;
		case 'resumenMovInvBodega':
			$controller->resumenMovInvBodega();
			break;
		case 'gsuministros':
			$controller->gsuministros();
			break;
		case 'verOCFallidas':
			$controller->verOCFallidas();
			break;
		case 'motivosFallidos':
			$oc=$_GET['oc'];
			$controller->motivosFallidos($oc);
			break;
		case 'verOrdenesFallidas':
			$controller->verOrdenesFallidas();
			break;
		case 'verDetalleOCF':
			$oc = $_GET['oc'];
