...
function ImprimeFacturaPegaso($factura, $destino){
        ...
        foreach($Detalle as $row){
			...
			$pdf->Ln(3);
			$pdf->SetFont('Arial','',6);
  			$pdf->Write(4, "Responsable del tratamiento de sus datos personales ".$DF->RAZON_SOCIAL." con dirrecion en: ".$DF->CALLE.', '.$DF->EXTERNO.', '.$DF->INTERNO.' Col: '.$DF->COLONIA.', CP: '.$DF->CP.", ".$DF->DELEGACION.', '.$DF->ESTADO.", correo electronico: telefono: .");
  			$pdf->Ln(4);	
			$pdf->Write(4, $DF->RAZON_SOCIAL.", Recaba, utiliza, almacena o trasfiere Datos Personales con apego a la ley, para los siguientes fines: hacer de su conocimiento , la gama de nuestros productos y servicios asi como los beneficios, descuentos, promociones y en general toda la publicidad derivada de los servicios que ofrecemos, efectuar la comercializacion y distribucion de todos nuestros productos y servicios. Para mayor informacion acerca del tratamiento y de los derechos que puede hacer valer, usted puede acceder al aviso de privacidad completo en la siguiente pagina: ");
  			$pdf->Ln(4);	
			$pdf->SetFont('Arial','',6);
  			$pdf->Write(4,'POR ESTE PAGARE DEBEMOS Y PAGAREMOS INCONDICIONALMENTE A LA ORDEN DE '.$DF->RAZON_SOCIAL.' LA CANTIDAD DE $ '.number_format(($subtotal-$descTot)*1.16,2).' IMPORTE RECIBIDO A MI ENTERA CONFORMIDAD, ESTE PAGARE DE NATURALEZA MERCANTIL Y ESTA REGIDO POR EL ARTICULO 173 Y DEMAS DE LA LEY GENERAL DE TITULO Y OPERACIONES DE CREDITO');
  			$pdf->Ln(6);
                    ...
                    if ($_FILES['files']['size'][$f] > $max_file_size) {
                        ...
                        $exec = $data->seleccionarArchivoXMLCargado($archivo, $a['uuid']); /// Cuando se selecciona el archivo para su alamcenamiento.
                        	if($exec==null){
                        	    ...
                        	    echo "<br/>Archivo $archivo <b>duplicado<b/>. No se ha logrado subir.<br/>";
                        	}	
                        }
                    }
                }
